...
Every update to `main` starts **App Release Android & iOS** for that exact commit.
It uploads one shared native version to TestFlight and Play `internal`, then writes
the attested `v<version>` tag. A successful native run from a `main` push starts
**App Release OTA** for the same commit. The OTA run publishes only while that commit
is still current `main`; a newer push gets its own native release and OTA attempt.
A manual native retry is available on `main`. The manual OTA workflow on `dev`
still builds the current `main` app tree with the reviewed `dev` release tooling.
Neither coordinated release builds the `dev` app tree.
...
| | button | what it does |
|-|--------|--------------|
| native | **App Release Android & iOS** | every `main` push resolves `<major>.<build+1>.0` → builds iOS + Android from that exact commit and one version → TestFlight + Play `internal` → tags `v<version>` |
...
The automatic native and OTA triggers become active only when these workflow files reach
`main`; merging this change into `dev` alone does not register `main` push or
`workflow_run` triggers. Updating `main` then runs native first and production OTA
after its success. To publish the existing `main` commit using the newer
...
`App Release OTA` builds and publishes a production static export after each successful
native release started by an update to `main`. It also accepts a manual dispatch on `dev`
to publish the current `main` source with the platform-aware release tooling. A dispatch on the
...
| ------- | ------- | -------------- |
| **App Release OTA** — successful native run from a `main` push | `ios-mobile-release` and `android-mobile-release` | `<major>.<build>.<ota+1>-ios` / `-android` |
...
3. Merge the reviewed commit to `main`. That push starts **App Release Android & iOS**.
   Its successful completion starts **App Release OTA** for the same main commit.
