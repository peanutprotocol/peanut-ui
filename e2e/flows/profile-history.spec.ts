...
test.describe('Profile and history pages', () => {
	...
	test('profile page — shows username and account info', async ({ browser }, testInfo) => {
		...
		const username = page.locator(
			'[data-test="username"], [class*="username"], text=/@\\w+/i'
		)
		if (await username.first().isVisible({ timeout: 3000 }).catch(() => false)) {
		...
		if (await accountInfo.first().isVisible({ timeout: 3000 }).catch(() => false)) {
	...
	test('points page — shows points balance', async ({ browser }, testInfo) => {
		...
		if (hasCrash) {
			...
			if (await pointsBalance.first().isVisible({ timeout: 5000 }).catch(() => false)) {
			...
			if (await pointsActivity.first().isVisible({ timeout: 3000 }).catch(() => false)) {
	...
	test('profile → history navigation', async ({ page }, testInfo) => {
		...
		const historyLink = page.locator(
			'a[href*="history"], button:has-text("History"), text=/view.*history/i'
		)
