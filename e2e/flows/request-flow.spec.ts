...
test.describe('Request pot flow', () => {
	...
	test('request link shows requester info and total amount', async ({ browser }, testInfo) => {
		...
		const requesterInfo = page.locator(
			'[data-test="requester-info"], [class*="requester"], text=/requested/i'
		)
		if (await requesterInfo.first().isVisible({ timeout: 3000 }).catch(() => false)) {
		...
		const amountDisplay = page.locator(
			'[data-test="request-amount"], [class*="amount"], text=/\\$|USD|USDC/i'
		)
		if (await amountDisplay.first().isVisible({ timeout: 3000 }).catch(() => false)) {
	...
	test('capture payment form state', async ({ browser }, testInfo) => {
		...
		const payButton = page.locator(
			'button:has-text("Pay"), button:has-text("Send"), button:has-text("Confirm")'
		)
		if (await payButton.first().isVisible({ timeout: 3000 }).catch(() => false)) {
