...
test.describe('Withdraw flow (expanded)', () => {
    ...
    test('withdraw landing — shows method selection', async ({ page, browser }, testInfo) => {
        ...
        const walletOption = activePage.locator('text=/wallet/i, button:has-text("Wallet"), button:has-text("Crypto")')
        const bankOption = activePage.locator('text=/bank/i, button:has-text("Bank")')
