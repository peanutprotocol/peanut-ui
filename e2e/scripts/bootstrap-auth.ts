...
async function bootstrap() {
	if (fs.existsSync(BOOTSTRAP_STATE_PATH) && !FORCE) {
		...
		console.log(
			`[bootstrap] To force re-creation: FORCE_BOOTSTRAP=true npx tsx e2e/scripts/bootstrap-auth.ts`
		)
		return
	}
	...
	try {
		...
		const confirmBtn = page.locator(
			'button:has-text("Confirm & finish"), button:has-text("Confirm")'
		)
		...
		await page.waitForFunction(
			() => {
				...
				for (const b of buttons) {
					if (
						(b.textContent?.includes('Confirm') || b.textContent?.includes('finish')) &&
						!b.disabled
					)
						return true
				}
		...
		for (let i = 0; i < 5; i++) {
			...
			if (await closeBtn.first().isVisible({ timeout: 1_500 }).catch(() => false)) {
		...
		const payload = JSON.parse(
			Buffer.from(jwtCookie.value.split('.')[1], 'base64url').toString()
		)
		...
		fs.writeFileSync(
			path.resolve(__dirname, '../.auth/bootstrap-failure.html'),
			html
		)
