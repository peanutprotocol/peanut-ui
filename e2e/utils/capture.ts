...
export function collectConsoleLogs(page: Page) {
    ...
    return {
        ...
        flush: (testInfo: TestInfo, name: string) => {
            if (entries.length > 0) {
                try {
                    writeFileSync(testInfo.outputPath(`${name}-console.json`), JSON.stringify(entries, null, 2))
                } catch {}
            }
            return entries
        },
...
export function getConsoleErrors(entries: Array<{ type: string; text: string }>) {
    return entries.filter((e) => e.type === 'error' && !CONSOLE_NOISE.some((p) => p.test(e.text)))
}
