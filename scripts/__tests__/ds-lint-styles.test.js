...
test('css scanning reads declarations and apply rules, not definitions, selectors or comments', () => {
    expect(
        ...
        `)
    ).toEqual({ apply: 'px-5\n bg-action-primary/20', declarations: '1rem\n#fff' })
})
...
test('counts each use and does not exempt a whole file containing a skeleton', () => {
    expect(countAlphaSemanticTokens('animate-pulse bg-foreground-primary/10\nbg-foreground-primary/10', tokens)).toBe(1)
...
test('the real ratchet rejects new css and typescript alpha uses outside globals.css', () => {
    ...
    try {
        ...
        writeFileSync(join(dir, 'src/styles/globals.css'), '@theme { --color-action-primary: #ff90e8; }')
        const run = (flag) =>
        ...
        writeFileSync(join(dir, 'src/extra.css'), '.test { @apply bg-action-primary/20 p-5 text-sm; color: #abc; }')
        writeFileSync(
            join(dir, 'src/example.tsx'),
            'export const Example = () => <div className="bg-action-primary/10" />'
        )
