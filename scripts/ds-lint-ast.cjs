 ...
 *
 * `null` when there is nothing to compose. Finite syntax branches and aliases
 * are all visited and composed through `per`; cycles are skipped by identity.
 */
function composeObjectCandidates(node, ctx, depth, seen, per) {
    if (!node) return null
    const found = resolveToObjectLiterals(node, ctx, depth, seen)
    let out = null
    for (const candidate of found.candidates) {
        const alts = per(candidate, { ...ctx, scopes: candidate.scopes })
        out = out ? union(out, alts) : alts
    }
    return out
}
...
function cvaAlternatives(node, ctx, depth, seen) {
    ...
    // The config itself can be conditional — `cva(base, cond ? A : {})` — so it
    // goes through the same multi-candidate path a conditional SPREAD does.
    // Reading it as a single object literal fell through to builder-object key
    // scanning, which returns the config's field NAMES and never its classes.
    const composed = composeObjectCandidates(config, ctx, depth, seen, (candidate, candidateCtx) =>
        cvaConfigAlternatives(candidate, candidateCtx, depth + 1, seen)
    )
    if (composed) return product(baseAlts, composed)
...
function cvaConfigCandidateAlternatives(configFields, variants, compounds, ctx, depth, seen) {
    ...
    for (const opaque of opaqueFields(configFields)) {
        ...
        const composed = composeObjectCandidates(opaque.value, opaque.ctx, depth, seen, (candidate, candidateCtx) =>
            cvaConfigAlternatives(candidate, candidateCtx, depth + 1, seen)
        )
    ...
    for (const { key } of effectiveFields(configFields)) {
        if (key === 'variants') {
            ...
            for (const opaque of opaqueFields(variantFields)) {
                ...
                // hide several, and unioning them read independently selected
                // axes as alternatives.
                const alts =
                    composeObjectCandidates(opaque.value, opaque.ctx, depth, seen, (candidate, candidateCtx) =>
    ...
    for (const { compound, choices } of compoundEntries(compounds, ctx, depth, seen)) {
        ...
        for (const opaque of opaqueFields(compoundFields)) {
            if (!opaque.value) continue
            // An ENTRY: only its `class` field renders, and it renders in BUILDER
            // mode so `class: ['a', 'b']` composes. Reading the whole entry in
            // lookup mode unioned its fields, which made two classes cva renders
            // together look mutually exclusive.
            const fromSpread =
                composeObjectCandidates(opaque.value, opaque.ctx, depth, seen, (candidate, candidateCtx) =>
                    compoundEntryClasses(candidate, candidateCtx, depth + 1, seen)
                ) ?? alternatives(opaque.value, opaque.ctx, depth + 1, seen, LOOKUP_OBJECT)
...
function* compoundEntries(table, ctx, depth, seen, choices = new Map()) {
    if (!table || seen.has(table.node)) return
    seen = new Set(seen)
    seen.add(table.node)
    const inner = { ...ctx, scopes: table.scopes }
    for (const element of table.elements) {
        if (ts.isSpreadElement(element)) {
            ...
            for (const [index, candidate] of found.candidates.entries()) {
                ...
                yield* compoundEntries(candidate, inner, depth, seen, next)
            }
        } else {
 ...
 */
function resolveToObjectLiterals(node, ctx, _depth, seen) {
    return resolveStructuralLiterals(node, ctx, seen, ts.isObjectLiteralExpression)
}

/** Preserve whole array candidates and their scopes, never pool their entries. */
function resolveToArrayLiterals(node, ctx, _depth, seen) {
    return resolveStructuralLiterals(node, ctx, seen, ts.isArrayLiteralExpression)
}

/**
 * Structural resolution is an iterative graph walk, bounded by distinct syntax
 * nodes rather than alias depth. A depth cutoff cannot safely summarize CVA
 * structures as generic classes: it loses selector/compound combinations.
 * Each const has a fixed declaring scope, so revisiting its initializer adds no
 * new candidate. Identity tracking also terminates cycles without dropping the
 * reachable literals on other branches.
 */
function resolveStructuralLiterals(node, ctx, seen, matches) {
    ...
    while (pending.length) {
        ...
        if (matches(node)) {
            candidates.push({
                node,
                scopes: ctx.scopes,
                ...(ts.isArrayLiteralExpression(node) ? { elements: [...node.elements] } : {}),
            })
        } else if (ts.isParenthesizedExpression(node) || ts.isAsExpression(node) || ts.isSatisfiesExpression(node)) {
            pending.push({ node: node.expression, ctx })
        } else if (ts.isConditionalExpression(node)) {
            pending.push({ node: node.whenFalse, ctx }, { node: node.whenTrue, ctx })
