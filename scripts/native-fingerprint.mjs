...
//   deps   Native plugin versions resolved from pnpm-lock.yaml, split into
//          Android, iOS and shared inputs. The generated Capacitor manifests are
//          committed but NOT regenerated by the OTA workflow, so a plugin bumped
//          in package.json + the lockfile without a `cap sync` leaves them stale:
//          `pnpm install` still hands the new JS wrapper to the bundle while the
//          manifests read unchanged. The lockfile is the authoritative record of
//          the wrapper being shipped.
//
...
// Most Capacitor plugins have native halves on both platforms and therefore
// default to shared. These two packages are the platform runtimes themselves:
// changing @capacitor/android cannot change the iOS binary, and vice versa. Keep
// exceptions explicit so a name heuristic can never make a dependency bump less
// restrictive by accident.
...
export const NATIVE_INPUTS = [
    ...
    // generated manifests, which the OTA workflow never regenerates. Platform
    // runtimes must not raise the other platform's floor; cross-platform plugins
    // remain shared and therefore count for both.
    { kind: 'deps', id: 'android/native-plugin-versions', platform: 'android' },
    { kind: 'deps', id: 'ios/native-plugin-versions', platform: 'ios' },
 ...
 * path-prefix filter: `capacitor.config.ts`, `patches/` and resolved shared-plugin
 * versions sit outside `android/` and `ios/` while describing the native half of
 * both, so a prefix test would quietly wave a shared plugin bump through for every
 * platform. Platform runtime versions have their own inputs. Every entry in
 * NATIVE_INPUTS carries an explicit `platform`, and a test asserts it, so a newly
 * added input cannot default to the lenient side.
 *
