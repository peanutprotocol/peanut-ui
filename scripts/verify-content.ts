...
function discoverRoutes(): Set<string> {
  ...
  const storySlugs = listDirs(path.join(CONTENT_DIR, "stories")).filter(
    (s) => s !== "index",
  );
  ...
  for (const locale of SUPPORTED_LOCALES) {
    ...
    if (hasRoute("use-cases/[slug]")) {
      for (const slug of useCaseSlugs)
        routes.add(`/${locale}/use-cases/${slug}`);
    }
...
function extractLinks(
  content: string,
): Array<{ line: number; url: string; text: string }> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while ((match = MARKDOWN_LINK_RE.exec(lineContent)) !== null) {
      if (isInternalLink(match[2]))
        links.push({ line: i + 1, url: match[2], text: match[1] });
    }
    JSX_HREF_RE.lastIndex = 0;
    while ((match = JSX_HREF_RE.exec(lineContent)) !== null) {
      if (isInternalLink(match[1]))
        links.push({ line: i + 1, url: match[1], text: "" });
    }
  }
  return links;
}
...
function checkPublishedHasRoute(validPaths: Set<string>) {
  ...
  for (const ct of contentTypes) {
    const slugs = listDirs(path.join(CONTENT_DIR, ct.dir)).filter(
      (s) => s !== "index",
    );
    for (const slug of slugs) {
      ...
      if (!validPaths.has(url)) {
        error(
          "no-route",
          `Published content has no route: ${url}`,
          rel(enFile),
        );
        issues++;
      }
  ...
  for (const intent of ["pricing", "supported-networks"]) {
    ...
    if (!validPaths.has(url)) {
      error(
        "no-route",
        `Published singleton has no route: ${url}`,
        rel(enFile),
      );
      issues++;
    }
  ...
  console.log(
    `  Pass 2 — Route coverage: ${issues === 0 ? "all published content has routes" : `${issues} issues`}`,
  );
}
...
function checkFooterManifest(validPaths: Set<string>) {
  ...
  function checkEntries(
    entries: Array<{ href: string; slug: string; external?: boolean }>,
    section: string,
  ) {
  ...
  if (manifest.sendMoney?.to)
    checkEntries(manifest.sendMoney.to, "sendMoney.to");
  if (manifest.sendMoney?.from)
    checkEntries(manifest.sendMoney.from, "sendMoney.from");
  ...
  console.log(
    `  Pass 3 — Footer manifest: ${issues === 0 ? "all URLs valid" : `${issues} broken URLs`}`,
  );
}
...
function checkFrontmatter() {
  ...
  console.log(
    `  Pass 4 — Frontmatter: ${issues === 0 ? "all published files valid" : `${issues} issues`}`,
  );
}
...
function checkLocaleCoverage() {
    ...
    for (const slug of slugs) {
      ...
      for (const locale of PRIMARY_LOCALES) {
        ...
        if (!fs.existsSync(path.join(slugDir, `${locale}.md`))) {
          warn(
            "locale",
            `${ct}/${slug}: missing ${locale} translation`,
            rel(enFile),
          );
          warnings++;
        }
  ...
  console.log(
    `  Pass 5 — Locale coverage: ${warnings} missing translations (warnings only)`,
  );
}
...
function checkContentPolish() {
  ...
  for (const file of files) {
    ...
    const mdxCount = MDX_COMPONENT_PATTERNS.filter((pattern) =>
...
function checkSubmoduleFreshness() {
  ...
  try {
    ...
    if (behind > 0) {
      warn(
        "submodule",
        `Content submodule is ${behind} commit(s) behind origin/main — consider bumping`,
      );
    }
    console.log(
