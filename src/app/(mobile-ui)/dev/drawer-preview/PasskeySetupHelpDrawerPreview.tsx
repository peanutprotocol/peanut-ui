...
// preview-only copy of the reverted PasskeySetupHelpModal→drawer conversion,
// for the TASK-21904 keep/change review. production stays a modal (handed to
// Slava, #2926 comment) — src/components/Setup/ is untouched; this duplicates
// its content into the shared Drawer with the built-in scroll area.

interface PasskeySetupHelpDrawerPreviewProps {
    visible: boolean
    onClose: () => void
    onRetry: () => void
    errorName: string
...
export const PasskeySetupHelpDrawerPreview = ({
    visible,
    onClose,
    onRetry,
    errorName,
    platform,
}: PasskeySetupHelpDrawerPreviewProps) => {
    ...
    // long troubleshooting content scrolls in DrawerContent's built-in scroll area
    return (
        <Drawer
            open={visible}
            onOpenChange={(isOpen) => {
            ...
            <DrawerContent>
                <div className="flex flex-col items-center gap-4 px-4 pt-1 pb-6 text-center">
                    ...
                    <div className="flex w-full flex-col gap-4 text-left">
                        <h2 className="mr-auto text-body-s text-foreground-secondary">{getDescription()}</h2>
