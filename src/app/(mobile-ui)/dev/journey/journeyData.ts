...
export const IN_APP_SURFACES: InAppSurface[] = [
    // ---------- 🏠 home activation steps (ActivationCTAs) ----------
    {
        ...
        cta: { label: 'row tap', dest: 'Unlock payments | Add drawer | first-payment chooser, /card or QR scanner' },
        condition:
            'Home until the first payment (resolveOnboarding, utils/activation-step.utils.ts). Verify: identity verified; "In review" pill while processing. Add money: any money received (milestone funded) or any wallet/collateral balance. First payment: API isActivated (card spend or QR pay); the row exists only when the card or a QR rail is open (selectFirstPaymentRoute), so a user with neither has three rows and completes at Add money. The first open, actionable row is outlined in pink. Replaced only by the region-restricted card or a provider rejection.',
    ...
    // ---------- 🏠 home carousel (activated users, 7d dismiss) ----------
    {
        ...
        cta: { label: 'tap', dest: '/profile/accounts-and-payments' },
