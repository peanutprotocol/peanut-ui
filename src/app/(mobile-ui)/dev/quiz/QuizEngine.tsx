...
import MySpaceChrome from './MySpaceChrome'
...
export default function QuizEngine({ quiz }: { quiz: QuizDefinition }) {
    ...
    const answeredWrong = chosen !== null && !answeredCorrectly
    const levelJustChanged =
        position > 0 && question !== undefined && quiz.questions[order[position - 1]].level !== question.level
    ...
    const answer = (originalIndex: number) => {
        ...
        if (originalIndex === question.correctIndex) {
            ...
            const gain = LEVEL_POINTS[question.level] + (newStreak >= 3 ? STREAK_BONUS : 0)
            setLastGain(gain)
            setPoints((p) => p + gain)
                        ...
                        >
                            ...
                            <Card className="p-4 text-center">
                                <h2 className="text-lg font-bold">
                                    {quiz.emoji} <span className="myspace-rainbow">{quiz.title}</span> {quiz.emoji}
                                </h2>
                                ...
                                <p className="myspace-blink myspace-comic mt-2 text-xs font-bold">
                                    ☆ pro tip: the peanut is clickable ☆
                                </p>
                            </Card>
                            <Button
                                variant="purple"
                    ...
                    {stage === 'playing' && question && (
                        <div className="flex flex-col gap-4">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="relative rounded-sm border border-n-1 bg-white px-2 py-0.5 text-xs font-bold">
                                    ⭐ {points}
                                    {lastGain > 0 && answeredCorrectly && (
                                        <span key={points} className="myspace-points-pop text-green-1">
                                            +{lastGain}
                                        </span>
                                    )}
                                </span>
                                <AnimatePresence>
                                    {streak >= 3 && (
                                        <motion.span
                                            ...
                                            animate={{ scale: 1 }}
                                            exit={{ scale: 0 }}
                                            className="myspace-comic rounded-sm border border-n-1 bg-yellow-1 px-2 py-0.5 text-xs font-bold"
                                        >
                                            🔥 COMBO x{streak}
                                            {streak >= 5 ? ' — UNSHELLED' : ''}
                                        </motion.span>
                            ...
                            {levelJustChanged && chosen === null && (
                                <div
                                    key={`levelup-${question.level}`}
                                    className="myspace-levelup-banner pointer-events-none fixed inset-x-0 top-1/3 z-50 text-center"
                                >
                                    <span className="myspace-comic inline-block rounded-sm border-2 border-n-1 bg-yellow-1 px-4 py-2 text-2xl font-extrabold shadow-lg">
                                        ⬆️ LEVEL UP: {LEVEL_LABEL[question.level]}
                                    </span>
                                </div>
                            )}
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                >
                                    ...
                                    <div className={`flex flex-col gap-2 ${answeredWrong ? 'myspace-shake-it' : ''}`}>
                        ...
                        >
                            ...
                            <Card className="p-4 text-center">
                                ...
                                <h2 className="mt-1 text-lg font-bold">
                                    <span className="myspace-rainbow">{grade.title}</span>
                                </h2>
