...
jest.mock('@/services/charges', () => ({
    // cancel gets a default impl: unrelated suites unmount the page, which
    // fires the draft-cleanup effect, and clearAllMocks keeps impls intact.
    chargesApi: { create: jest.fn(), get: jest.fn(), cancel: jest.fn(() => Promise.resolve()) },
}))
...
jest.mock('@/components/Global/ActionModal', () => ({
    __esModule: true,
    // Exposes onClose so the draft-cleanup suite can dismiss the
    // compatibility modal; renders nothing when hidden, like before.
    default: (props: { visible?: boolean; onClose?: () => void }) =>
        props.visible ? <button data-testid="modal-close" onClick={props.onClose} /> : null,
}))
...
// Ported from #3063 (TASK-21087) onto the pre-tech-debt page shape: the
// stepper became currentView, the review CTA testid is initial-view, and
// Back is exercised by flipping currentView off CONFIRM (the watcher owns
// draft abandonment for every exit path).
describe('unpaid withdrawal draft cleanup', () => {
    beforeEach(() => {
        mockWithdrawFlow.currentView = 'INITIAL'
        jest.mocked(chargesApi.create).mockResolvedValue({ data: { id: CHARGE_UUID } } as never)
        jest.mocked(chargesApi.get).mockResolvedValue(chargeDetails as never)
        jest.mocked(chargesApi.cancel).mockResolvedValue(undefined as never)
    })
    ...
    it('cancels when leaving CONFIRM and refuses a stale Confirm handler', async () => {
        const view = render(<WithdrawCryptoPage />)
        fireEvent.click(screen.getByTestId('initial-view'))
        await waitFor(() => expect(chargesApi.get).toHaveBeenCalled())
        mockWithdrawFlow.currentView = 'CONFIRM'
        ...
        mockWithdrawFlow.currentView = 'INITIAL'
        view.rerender(<WithdrawCryptoPage />)
        await waitFor(() => expect(chargesApi.cancel).toHaveBeenCalledTimes(1))
        mockWithdrawFlow.currentView = 'CONFIRM'
        view.rerender(<WithdrawCryptoPage />)
        fireEvent.click(screen.getByTestId('confirm-withdraw'))
        expect(mockSendMoney).not.toHaveBeenCalled()
