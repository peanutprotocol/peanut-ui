...
/**
 * Metadata for a payment link, shared by the root `/<recipient>` catch-all and the
 * `/pay/<recipient>` route those links are now printed under. Both routes render
 * the same page, so both must produce the same OG card — a shared request link is
 * previewed in WhatsApp/X/Telegram before anyone taps it.
 */
export async function buildPaymentMetadata(segments: string[] | undefined, chargeId?: string) {
    ...
    let recipient = segments[0].toLowerCase()
    ...
    if (segments[1]) {
        try {
            const parsed = parseAmountAndToken(segments[1])
    ...
    const isPeanutUsername = segments.length === 1 && !isEthAddress && !isEnsName
