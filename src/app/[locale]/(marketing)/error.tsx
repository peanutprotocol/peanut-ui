...
export default function MarketingError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
    ...
    return (
        <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center">
            <h1 className="text-heading-s">{i18n.title}</h1>
            <p className="mt-2 max-w-md text-foreground-secondary">{i18n.body}</p>
            <div className="mt-6 flex gap-3">
                <button
                    onClick={reset}
                    className="rounded-md bg-black px-4 py-2 text-body-s text-white hover:bg-gray-800"
                >
                    {i18n.tryAgain}
                </button>
                <Link
                    href={locale === DEFAULT_LOCALE ? '/' : `/${locale}`}
                    className="rounded-md border border-gray-300 px-4 py-2 text-body-s hover:bg-gray-100"
                >
                    {i18n.goHome}
                </Link>
            </div>
