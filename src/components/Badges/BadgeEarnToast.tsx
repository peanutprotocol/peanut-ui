 ...
 * coalesced toast ("Badge unlocked: X" / "You unlocked N badges"). A
...
export default function BadgeEarnToast() {
    ...
    useEffect(() => {
        ...
        // Announce avatars only when one of the earned badges has art.
        ...
        if (avatarCount > 0) {
            const avatarTimeout = setTimeout(() => {
                ...
                toast({
                    ...
                    className: 'bg-background-default',
                    content: (
                        <span className="text-left text-label-l">{t('toastAvatars', { count: avatarCount })}</span>
                    ),
                })
        ...
        markSeen(codes)
    }, [pathname, pending, toast, markSeen, t, badgeCopy])
    ...
    useEffect(() => {
        const timeouts = avatarTimeoutsRef.current
        return () => {
