...
// The `badges` namespace translator (useTranslations('badges')) — passed into
// getBadgeShareText so the share copy lives in the locale catalogs while the
// composition stays here, shared by the detail modal and the unlock drawer.
type BadgesTranslator = ReturnType<typeof useTranslations<'badges'>>
...
//
// The per-badge brags live in the catalogs under `badges.share.lines.<CODE>`
// (en is the source; es-419/pt-BR full, es-AR voseo deltas). Rarely-earned badges
// are intentionally absent there (e.g. CERTIFIED_YAPPER, TOKEN_NATION_SP_2026,
// FESTA_JUNINA_2026) — t.has() falls back to the generic brag for any unlisted
// code, so a badge almost nobody has needs zero copy upkeep.
export function getBadgeShareText(
    ...
    const lineKey = `share.lines.${code}` as Parameters<BadgesTranslator>[0]
    const brag = code && t.has(lineKey) ? t(lineKey) : t('share.generic', { badge: displayName })
    return `${brag}\n\n${t('share.join', { link: profileUrl })}`
}
