...
// Wrapper goes INSIDE ui: renderWithIntl spreads its options last, so a
// `wrapper` option would replace IntlWrapper instead of composing with it.
const renderLock = () =>
    render(
        <Wrapper>
            <LockCardModal cardId="card-1" mode="lock" isOpen onClose={jest.fn()} />
        </Wrapper>
    )
const renderCancel = () =>
    render(
        <Wrapper>
            <CancelCardModal cardId="card-1" isOpen onClose={jest.fn()} />
        </Wrapper>
    )
