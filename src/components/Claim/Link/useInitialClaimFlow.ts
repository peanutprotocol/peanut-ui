...
export const useInitialClaimFlow = (props: IClaimScreenProps, campaignTag: string | undefined) => {
    ...
    // all query params this hook reads, via nuqs (DS 10 ratchet) — read-only,
    ...
    const {
        ...
        tokenPrice,
        setClaimType,
        // verbatim-move artifact: the view renders attachment from props itself
    ...
    const handleIbanRecipient = async () => {
        try {
            ...
            setLoadingState('Checking your verification')
    ...
    // recipient-input onUpdate handler — moved verbatim from the inline JSX closure
    ...
    }, [user, stepFromURL, methodFromURL, isPeanutWallet, selectedTokenData, address])
