...
describe('ActionModal tone', () => {
    ...
    it("peanut is the yellow bubble around the given icon: Peanut's own", () => {
        const { container } = renderModal({ tone: 'peanut', icon: 'bell' })
        expect(bubble(container)).toHaveClass('bg-background-icon-bubble-yellow')
        expect(bubble(container)!.querySelector('.lucide-bell')).not.toBeNull()
    })

    it("a concept draws that concept's bubble: QR pay is pink", () => {
        const { container } = renderModal({ concept: 'qrPay' })
        expect(bubble(container)).toHaveClass('bg-background-brand')
        expect(bubble(container)!.querySelector('.lucide-qr-code')).not.toBeNull()
    })
