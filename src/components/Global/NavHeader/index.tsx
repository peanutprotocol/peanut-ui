...
const NavHeader = ({ title, icon = 'chevron-up', href, hideLabel = false, onPrev, disableBackBtn }: NavHeaderProps) => {
    return (
        <div className="relative flex w-full flex-row items-center justify-between md:block">
            ...
            {!hideLabel && (
                <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform pb-1 text-2xl font-extrabold md:relative md:left-auto md:top-auto md:hidden md:translate-x-0 md:translate-y-0 md:transform-none md:pb-0 md:text-base md:font-semibold">
                    {title}
                </div>
            )}
        </div>
