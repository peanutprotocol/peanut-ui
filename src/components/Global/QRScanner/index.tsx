...
// higher resolution for dense qr codes (mercado pago, pix emvco)
const MIN_CAMERA_WIDTH = 1280
const MIN_CAMERA_HEIGHT = 720
const IDEAL_CAMERA_WIDTH = 1920 // request 1080p for better dense qr detection
const IDEAL_CAMERA_HEIGHT = 1080
const MAX_CAMERA_WIDTH = 4096 // allow up to 4k if device supports
const MAX_CAMERA_HEIGHT = 2160
...
export default function QRScanner({ onScan, onClose, isOpen = true }: QRScannerProps) {
    ...
    const startCamera = useCallback(async () => {
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            try {
                ...
                // Request camera with specified facing mode and higher resolution for dense qr codes
                const stream = await navigator.mediaDevices.getUserMedia({
                    video: {
                        facingMode,
                        width: { min: MIN_CAMERA_WIDTH, ideal: IDEAL_CAMERA_WIDTH, max: MAX_CAMERA_WIDTH },
                        height: { min: MIN_CAMERA_HEIGHT, ideal: IDEAL_CAMERA_HEIGHT, max: MAX_CAMERA_HEIGHT },
                    },
                })
    ...
    // paste functionality from PR 1638
        ...
        <div className="qr-scanner-container fixed left-0 top-0 z-50 flex h-full w-full flex-col bg-black">
            {error ? (
                <div className="p-4 text-center text-white">
                    ...
                    <Button onClick={closeScanner} variant="purple" shadowSize="4" className="mt-4">
                        Close
                    </Button>
                </div>
            ) : (
                <>
                    ...
                    <div className="fixed left-1/2 flex h-64 w-64 -translate-x-1/2 translate-y-1/2 justify-center">
                        ...
                        <div className="flex-column z-50 translate-y-[100%] transform items-center text-center">
                            ...
                            <button
                                onClick={handlePaste}
                                className="mx-auto mt-6 flex items-center gap-1.5 text-center text-white underline underline-offset-2"
                            >
