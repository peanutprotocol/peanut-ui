...
/** Slug → catalog key under `global.reConsent.docs` plus its reader route. */
const DOC_LABELS: Record<string, { nameKey: DocNameKey; href: string }> = {
    terms: { nameKey: 'terms', href: '/terms' },
    privacy: { nameKey: 'privacy', href: '/privacy' },
    'card-terms-us': { nameKey: 'cardTermsUs', href: '/card-terms-us' },
    'card-terms-international': { nameKey: 'cardTermsInternational', href: '/card-terms-international' },
    'card-esign': { nameKey: 'cardEsign', href: '/card-esign' },
    'card-privacy': { nameKey: 'cardPrivacy', href: '/card-privacy' },
    'card-prohibited-activities': { nameKey: 'cardProhibitedActivities', href: '/card-prohibited-activities' },
}
...
const ReConsentModal = () => {
    const t = useTranslations('global.reConsent')
        ...
        <ActionModal
            ...
            content={
                <div className="w-full space-y-3 text-left">
                     ...
                     * date (see utils.ts). */}
                    <p className="text-sm text-grey-1">{t('bodyReassurance')}</p>
                    <p className="text-sm text-grey-1">{t('bodyWhatChanged')}</p>
                    <ul className="space-y-1 text-sm">
                        {outdatedDocs.map((doc) => {
                            const label = DOC_LABELS[doc.slug]
                            return (
                                <li key={doc.slug}>
                                    <DocsLink
                                        href={label?.href ?? `/${doc.slug}`}
                                        className="text-black underline dark:text-white"
                                    >
                                        {label ? t(`docs.${label.nameKey}`) : doc.slug}
                                    </DocsLink>
                                </li>
                            )
                        })}
            ...
            ctas={[
                {
                    text: submitting ? tCommon('saving') : t('ctaAccept'),
                    variant: 'purple',
                    shadowSize: '4',
