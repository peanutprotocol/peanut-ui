...
const KycPrepChecklist = ({ path }: { path: KycPrepPath }) => {
    ...
    return (
        <div className="flex w-full flex-col gap-3 text-left" data-testid="kyc-prep-checklist">
            <p className="text-sm">{t(`intro.${path}`)}</p>
            <div className="overflow-hidden rounded-sm border border-n-1 dark:border-white">
                {items.map((item) => (
                    <div
                        key={item}
                        className="flex items-start gap-2 border-t border-n-1 p-3 first:border-t-0 dark:border-white"
                    >
                        <Icon name="check-circle" className="mt-0.5 size-4 shrink-0" />
                        <span>
                            <span className="block text-sm font-bold">{t(`items.${item}.title`)}</span>
                            <span className="block text-xs text-grey-1">{t(`items.${item}.body`)}</span>
                        </span>
                    </div>
                ))}
            </div>
            <div className="rounded-sm border border-n-1 p-3 dark:border-white">
                <span className="block text-[10px] font-bold tracking-wide uppercase">{t('howLongLabel')}</span>
                <span className="block text-xs">{t(`howLong.${path}`)}</span>
            </div>
            <p className="text-xs text-grey-1">{t('extraDocNote')}</p>
        </div>
    )
}
