...
type CardPioneersVariant = 'centered' | 'split-left' | 'split-right'

interface CardPioneersProps {
    variant?: CardPioneersVariant
}

const CardPioneers = ({ variant = 'split-left' }: CardPioneersProps) => {
    const { user } = useAuth()
    ...
    if (variant === 'centered') {
        return (
            <section className="relative overflow-hidden bg-secondary-1 py-16 text-n-1 md:py-24">
                {!isMobile && <Stars />}
                <div className="relative flex flex-col items-center justify-center px-4">
                    <h1 className="font-roboto-flex-extrabold text-center text-[2.5rem] font-extraBlack md:text-6xl lg:text-headingMedium">
                        PAY EVERYWHERE ELSE.
                    </h1>

                    <h2 className="font-roboto-flex mt-4 text-center text-lg md:text-3xl">
                        Latam with best in class QR payments. For everywhere else, use the Peanut Card.
                    </h2>

                    <PioneerCard3D className="my-10" />

                    <div className="font-roboto-flex space-y-2 text-center text-base md:text-lg">
                        <p>Best rates - no hidden fees</p>
                        <p>Earn forever for every invite</p>
                        <p>Self-custodial - your funds, your control</p>
                    </div>
                    ...
                    <div className="mt-8 flex flex-col items-center gap-3">
                        <Button
                            ...
                            shadowSize="4"
                            onClick={handleCTA}
                            className="px-10 py-4 text-lg font-extrabold"
                        >
                            GET MY CARD
                        </Button>
                        <a
    ...
    // Variant B: Split layout - Card on left, copy on right
                ...
                <div className="relative mx-auto flex max-w-7xl flex-col items-center gap-8 px-4 md:flex-row md:gap-12 md:px-8 lg:gap-20">
                    ...
                    <div className="w-full text-center md:w-1/2 md:text-left">
                        ...
                        <p className="font-roboto-flex mt-4 text-lg md:text-xl">
                            Latam with best in class QR payments. For everywhere else, use the Peanut Card and get
                            rewarded.
                        </p>
                        ...
                        <ul className="font-roboto-flex mt-6 space-y-3 text-base md:text-lg">
                            <li className="flex items-center justify-center gap-3 md:justify-start">
                                <span className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-n-1 text-sm text-white">
                                    ✓
                                </span>
    ...
    // Variant C: Split layout - Copy on left, card on right
    return (
        <section className="relative overflow-hidden bg-secondary-1 py-16 text-n-1 md:py-24">
            {!isMobile && <Stars />}
            <div className="relative mx-auto flex max-w-7xl flex-col-reverse items-center gap-8 px-4 md:flex-row md:gap-12 md:px-8 lg:gap-20">
                {/* Copy on left */}
                <div className="w-full text-center md:w-1/2 md:text-left">
                    <h1 className="font-roboto-flex-extrabold text-[2.25rem] font-extraBlack leading-tight md:text-5xl lg:text-6xl">
                        PAY EVERYWHERE ELSE.
                    ...
                    <p className="font-roboto-flex mt-4 text-lg md:text-xl">
                        Latam with best in class QR payments. For everywhere else, use the Peanut Card and get rewarded.
                    </p>
