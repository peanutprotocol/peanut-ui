...
export const mdxComponents: Record<string, React.ComponentType<any>> = {
    ...
    p: (props: React.HTMLAttributes<HTMLParagraphElement>) => (
        <p
            className={`mx-auto mb-6 ${PROSE_WIDTH} px-6 text-base leading-[1.75] text-grey-1 md:px-4`}
            {...props}
        />
    ),
    a: ({ href = '', ...props }: React.AnchorHTMLAttributes<HTMLAnchorElement>) => (
        <Link href={href} className="text-n-1 underline decoration-n-1/30 underline-offset-2 hover:decoration-n-1" {...props} />
    ),
    ul: (props: React.HTMLAttributes<HTMLUListElement>) => (
        <ul
            className={`mx-auto my-5 ${PROSE_WIDTH} list-disc space-y-3 pl-12 pr-6 md:pl-10 md:pr-4`}
            {...props}
        />
    ),
    ol: (props: React.HTMLAttributes<HTMLOListElement>) => (
        <ol
            className={`mx-auto my-5 ${PROSE_WIDTH} list-decimal space-y-3 pl-12 pr-6 md:pl-10 md:pr-4`}
            {...props}
        />
    ),
    ...
    th: (props: React.HTMLAttributes<HTMLTableCellElement>) => (
        <th className="border-b border-n-1 bg-primary-1/15 px-4 py-3 text-xs font-bold uppercase tracking-wide text-n-1" {...props} />
    ),
