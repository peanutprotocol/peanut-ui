...
/**
 * Mid-sentence forms of the generic labels the receipt title interpolates
 * ("Added from bank account", not "Added from Bank account"). The `name.*`
 * form stays capitalised because rows and the feed show it standalone. Real
 * counterparty names never pass through here, so proper nouns keep their case.
 */
export const IN_SENTENCE_NAME_KEYS = {
    ...
    [TRANSACTION_NAME_KEYS.bankAccount]: 'nameInSentence.bankAccount',
    [TRANSACTION_NAME_KEYS.recipient]: 'nameInSentence.recipient',
    [TRANSACTION_NAME_KEYS.sender]: 'nameInSentence.sender',
    [TRANSACTION_NAME_KEYS.peanutReward]: 'nameInSentence.peanutReward',
    [TRANSACTION_NAME_KEYS.transaction]: 'nameInSentence.transaction',
} as const satisfies Partial<Record<TransactionNameKey, string>>
