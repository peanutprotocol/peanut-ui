...
export function SendInitialView({ onNextScreen, setClaimLink, setTxReceipt, setChainId }: _consts.ISendScreenProps) {
    ...
    const customChainOption = ({ value, label, logoUri }: { value: number; label: string; logoUri: string }) => (
        <div>
    ...
    return (
        <>
            <div className="mt-6 flex  w-full flex-col gap-5 text-center ">
                <h2 className="title-font bold m-0 text-3xl lg:text-5xl">
                    Send crypto with a link
                    <span className="ml-2 text-lg font-bold text-teal lg:text-2xl">BETA</span>
                </h2>
                <h3 className="m-0 text-lg font-bold lg:text-2xl">Peanut Protocol Demo</h3>

                <div className="m-0 mx-auto w-11/12 p-5 px-6 text-base lg:w-2/3">
            ...
            <form className="w-full" onSubmit={sendForm.handleSubmit(createLink)}>
                <div className="flex w-full flex-col items-center gap-5">
                    <div className="flex w-full gap-2 px-2 sm:w-3/4 lg:w-3/5">
                        <div className="relative w-full lg:max-w-sm">
                            <Select
                                noOptionsMessage={() => 'no chains found'}
                                value={{
                                    value: formwatch.chainId,
                                    label: chainDetails.find((chain) => chain.chainId == formwatch.chainId)?.name ?? '',
                                    logoUri: '',
                                }}
                                ...
                                options={chainDetails.map((detail) => {
                                    return {
                                        ...
                                        label: detail.name,
                                        logoUri: detail.hasOwnProperty('icon') ? detail.icon[0].url : '',
                                    }
                                })}
                                formatOptionLabel={customChainOption}
                                onChange={(option) => {
                                    setFormHasBeenTouched(true)
                                    if (option && option.value) sendForm.setValue('chainId', option.value)
                                }}
                                isSearchable={false}
                            />
                        </div>
                        <div className="relative w-full lg:max-w-sm">
                            <Select
                                ...
                                onChange={(option) => {
                                    setFormHasBeenTouched(true)
                                    if (option && option.value) sendForm.setValue('token', option.value)
                                }}
                                isSearchable={false}
                            />
                        </div>
                            ...
                            <div className="absolute inset-y-0 right-4 box-border flex items-center ">
                                <span className="cursor-pointertext-lg align-center flex h-1/2 ">
                                    <button
                                        type="button"
                                        className={
                                            'color-white relative inline-flex h-full min-w-75 items-center  justify-center  border-2 border-black bg-black p-1 text-white sm:p-2'
                                        }
                                    >
                                        {formwatch.token}
                                    </button>
