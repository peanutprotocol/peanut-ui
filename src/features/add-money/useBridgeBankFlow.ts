...
import type { BridgeBankStep } from './types'

/**
 * flow hook for the Bridge SEPA bank deposit page — owns every behaviour so
 * the view stays dumb (same model as features/home/useHomeFlow). Only mounted
 * for non-Manteca countries — AddMoneyBankPage bounces BR/AR away before this
 * ever runs, so none of its data hooks / URL-state effects run for a Manteca
 * deep link.
 */
