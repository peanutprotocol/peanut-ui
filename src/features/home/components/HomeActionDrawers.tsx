...
type AddMethodKey = 'bankTransfer' | 'crypto' | 'migrateFromOfframp' | 'migrateFromOfframpDescription'
...
interface DrawerOption {
    ...
    /** body text living in addMoney.methods instead of home.drawers */
    methodsBodyKey?: AddMethodKey
    icon: IconName
    href: string
}
...
export function HomeActionDrawers() {
    ...
    const { user } = useAuth()
    // parity with /add-money (F-23): an OFFRAMP_USER who taps Add on home must
    // see the same migration entry the add-money method list grants them
    const hasOfframpMigrationEntry = user?.user?.badges?.some((badge) => badge.code === 'OFFRAMP_USER') ?? false
                ...
                {content && (
                    <div className="flex flex-col gap-4">
                        ...
                        <div className="flex flex-col">
                            {(content === 'add' && hasOfframpMigrationEntry
                                ? [
                                      {
                                          key: 'offramp-migration',
                                          titleKey: ['methods', 'migrateFromOfframp'],
                                          methodsBodyKey: 'migrateFromOfframpDescription',
                                          icon: 'wallet-outline',
                                          href: OFFRAMP_MIGRATION_ROUTE,
                                      } as DrawerOption,
                                      ...DRAWER_OPTIONS.add,
                                  ]
                                ...
                                <ListItem
                                    ...
                                    body={
                                        option.methodsBodyKey
                                            ? tMethods(option.methodsBodyKey)
                                            : option.bodyKey
                                              ? t(option.bodyKey)
                                              : undefined
                                    }
