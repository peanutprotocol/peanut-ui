...
import { isPriceLockExpired, receivePriceLock } from '../price-lock'
...
describe('receivePriceLock', () => {
    it('counts the remaining time from when the answer arrived, not from expiresAt', () => {
        const lock = receivePriceLock({ ...LOCK, expiresInMs: 120_000 }, RECEIVED_AT)

        expect(isPriceLockExpired(lock, RECEIVED_AT + 119_000)).toBe(false)
        expect(isPriceLockExpired(lock, RECEIVED_AT + 120_000)).toBe(true)
    })

    it('without a remaining time there is no device deadline: the provider decides', () => {
        const lock = receivePriceLock(LOCK, RECEIVED_AT)

        expect(lock.deadline).toBeUndefined()
        expect(isPriceLockExpired(lock, RECEIVED_AT + 10 * 60_000)).toBe(false)
    })
})
