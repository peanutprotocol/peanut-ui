...
const setLiveRate = (rate: string) =>
    act(() => {
        queryClient.setQueryData(['bridgeOfframpQuote', 'mxn', null], rateQuote('mxn', rate))
    })

beforeEach(() => {
    jest.clearAllMocks()
    mockGetOfframpQuote.mockResolvedValue({ data: rateQuote('mxn', '17') })
})
...
describe('Rates & fees USD amount → a saved CLABE (MXN amount step)', () => {
    ...
    it('waits for a delayed quote, then opens on the same seed', async () => {
        let resolveQuote!: (value: { data: ReturnType<typeof rateQuote> }) => void
        ...
        await act(async () => resolveQuote({ data: rateQuote('mxn', '17') }))
        await waitFor(() => expect(field().value).toBe('10'))
    })
...
describe('Rates & fees USD amount → a saved EUR account, then Back from the review', () => {
    const setEurRate = (rate: string) =>
        act(() => {
            queryClient.setQueryData(['bridgeOfframpQuote', 'eur', null], rateQuote('eur', rate))
        })
    const openSavedIban = async () => {
        mockGetOfframpQuote.mockResolvedValue({ data: rateQuote('eur', '0.9') })
