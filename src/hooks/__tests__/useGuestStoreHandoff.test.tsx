...
jest.mock('@/components/Migration/ScanToDownloadModal', () => ({
    ...
    default: ({ surface, handoff }: { surface: string; handoff?: { dest?: string } }) => (
        <div data-testid="scan-modal" data-surface={surface} data-dest={handoff?.dest ?? ''} />
    ),
}))
...
describe('useGuestStoreHandoff surface', () => {
    ...
    /* the modal takes the hand-off itself, not a pre-built payload: DownloadQR
     * derives the querystring from it after mount and gives the same one to the
     * store buttons under the code, so the two can never disagree. */
    it('carries the caller handoff into the desktop QR', () => {
        const { result, rerender } = renderHookWithIntl(() =>
            useGuestStoreHandoff({ surface: MIGRATION_SURFACES.LANDING_DOOR })
        )
        ...
        const modal = result.current.storeHandoffModal as ReactElement<{ handoff?: { dest?: string } }>
        expect(modal.props.handoff).toEqual({ dest: '/card' })
    })

    it('leaves the desktop QR context-free when the caller passes no handoff', () => {
        ...
        const modal = result.current.storeHandoffModal as ReactElement<{ handoff?: { dest?: string } }>
        expect(modal.props.handoff).toBeUndefined()
    })
