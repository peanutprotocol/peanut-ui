...
/** A top-up of the user's own wallet: the only onramp the app creates. */
export type CreateOnrampParams = {
    country: CountryData
    /** The typed amount in the rail's currency. */
    amount: string
}
    ...
    const createOnramp = useCallback(async ({ amount, country }: CreateOnrampParams) => {
        ...
        try {
            ...
            const response = await apiFetch('/bridge/onramp/create', {
                method: 'POST',
                body: JSON.stringify({
                    // Always a 2-decimal string: the API accepts only that
                    // shape, and the amount field keeps "100." while typing.
